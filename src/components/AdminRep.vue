<template>
  <v-card :id="'card' + id" style="background-color: white; margin: 15px; direction: rtl" class="mx-auto font">
    <v-list class="list-style" three-line>
        <v-list-item>
          <v-list-item-avatar  style="width:100px;height: 100px; border-radius: 50%">
          ...
          <v-list-item-content style="text-align: right; margin: 0 20px 0 20px">
            <v-list-item-title class="font_name" style="font-weight: bolder">رقم الرسالة : {{ num }}
            </v-list-item-title>
            <v-list-item-subtitle class="font_age" style="font-weight: bolder">رقم البلاغ : {{ id }}
          ...
          <div class="text-center">
            <v-dialog v-model="dialog" width="500">
              <template v-slot:activator="{ on, attrs }">
                ...
                <font-awesome-icon :id="id" @click="change(id)" title="رؤية اكثر" style="
                    color: #e9bba1;
                    cursor: pointer;
                    font-size: 37px;
                    margin-right: 15px;
                    margin-bottom: -8px;
                  " :icon="showIcon" />
                <font-awesome-icon @click="reverseChange(id)" :id="id + 'show'" title="رؤية اقل" style="
                    display: none;
                    color: #e9bba1;
                    cursor: pointer;
                    font-size: 37px;
                ...
                <v-icon @click="takeAction(id, 2)" style="color: #e05b0e; font-size: 27px; margin-right: 15px" depressed>
                  > mdi-delete
                </v-icon>
              </template>
              ...
              <v-card class="font">
                ...
                <v-card-actions>
                  ...
                  <v-btn @click="
                    dialog = false;
                  takeAction(id, 3);
                  " style="background-color: #e05b0e; color:white; font-size: 1.15rem; font-weight: bolder;">
                    منع
                  </v-btn>
                  <v-btn @click="
                    dialog = false;
                  takeAction(id, 4);
                  "  text style="margin-left: 7px; color: #eaaf8d;">
                    منع لمدة محددة
                  </v-btn>
                </v-card-actions>
              </v-card>
