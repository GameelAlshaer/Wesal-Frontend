<template>
  <v-card :id="id" style="background-color: white; margin: 15px; direction: rtl" class="mx-auto hover">
          ...
          <div class="text-center">
            <v-dialog v-model="dialog" width="500">
              <template v-slot:activator="{ on, attrs }">
                <v-icon v-bind="attrs" v-on="on" style="color: red; font-size: 27px" depressed>
                  > mdi-delete
                </v-icon>
                <v-icon style="
                    cursor: pointer;
                    font-size: 27px;
                    color: #985d4b;
                  " @click="redirect" title="صفحته الشخصية">
                  mdi-account
                </v-icon>
              </template>
